...
BD-fxn-db-to-lookup.pl: FROM THE COMMAND LINE, Create a MLDBM lookup file
...
if ($options{table} =~ /uniref/i){
  $sel_qry = qq|SELECT u.acc, u.desc, u.organism, u.kegg_acc, u.cog_acc, u.seed_acc, u.aclame_acc,
