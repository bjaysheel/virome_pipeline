...
while (my $line = <$ifh>) {
    ...
    if($line =~ /^#/) {
        ...
        if($seq_id eq '' || $seq_id eq $fields[0]) {
            $seq_id = $fields[0];
            # Are we in an epitope?
            if($fields[8] eq 'E') {
                # Start a new eptiope here
                if(!$in_epitope) {
                    $in_epitope = 1;
                    $epitope_start=$fields[3];
                    $epitope_stop=$fields[3];
                }
                # If we are we'll extend.
                else {
                    $epitope_stop = $fields[4];
                }
            }
            
            # If we're not in an epitope we'll go in here if we were 
            # just recently in one.
            elsif($in_epitope) {
                if(!$epitope_hash->{$seq_id}) {
                    $epitope_hash->{$seq_id} = ();
                }
...
sub create_doc {
    ...
    foreach my $s (keys %$epitopes) {
        ...
        if ( scalar @{$epitopes->{$s}} ) {
            $idgen->set_pool_size( located_sequence_feature => scalar @{$epitopes->{$s}} );
        }
        
        foreach my $ep_ref (@{$epitopes->{$s}}) {
