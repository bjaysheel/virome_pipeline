...
sub expand {
    ...
    foreach my $seqline(@seqarray){
	  ...
	  if(defined $u_hash->{kegg_acc} && (length($utils->trim($u_hash->{kegg_acc})) > 1) && ($u_hash->{kegg_acc} !~ /null/i)){
		  #reset array.
		  @arr1 = ();
		  @arr1 = split(/[\t]/,$seqline);
		  ...
		  if ((!$kFxn) && (defined $k_hash->{fxn1}) && ($k_hash->{fxn1} !~ /unknown|unclassified|unassigned|uncharacterized/i) && length($k_hash->{fxn1})){
			$kegg_row .= "\t1\n";
			$kFxn=1;
		  } else { $kegg_row .= "\t0\n"; }
	  ...
	  $resultrow = "";
	  if(defined $u_hash->{cog_acc} && (length($utils->trim($u_hash->{cog_acc})) > 1) && ($u_hash->{cog_acc} !~ /null/i)){
		  #reset array.
		  @arr1 = ();
		  @arr1 = split(/[\t]/,$seqline);
		  ...
		  # if acc has meaning full fxn 1 then set fxnal hit at 1 else 0
		  if ((!$cFxn) && (defined $c_hash->{fxn1}) && ($c_hash->{fxn1} !~ /unknown|unclassified|unassigned|uncharacterized/i) && length($c_hash->{fxn1})){
			$cog_row .= "\t1\n";
			$cFxn=1;
		  } else { $cog_row .= "\t0\n"; }
	  ...
	  if(defined $u_hash->{seed_acc} && (length($utils->trim($u_hash->{seed_acc})) > 1) && ($u_hash->{seed_acc} !~ /null/i)){
		  #reset array.
		  @arr1 = ();
		  @arr1 = split(/[\t]/,$seqline);
		  ...
		  # if acc has meaning full fxn 1 then set fxnal hit at 1 else 0
		  if ((!$sFxn) && (defined $s_hash->{fxn1}) && ($s_hash->{fxn1} !~ /unknown|unclassified|unassigned|uncharacterized/i) && length($s_hash->{fxn1})){
			$seed_row .= "\t1\n";
			$sFxn=1;
		  } else { $seed_row .= "\t0\n"; }
	  ...
	  if(defined $u_hash->{phgseed_acc} && (length($utils->trim($u_hash->{phgseed_acc})) > 1) && ($u_hash->{phgseed_acc} !~ /null/i)){
                  #reset array.
	      @arr1 = ();
	      @arr1 = split(/[\t]/,$seqline);
	      ...
	      my @p_arr = split(/;/,$u_hash->{phgseed_acc});
                  ...
                  # get acc lookup
	      my $p_acc_hash = $utils->get_acc_from_lookup("phgseed",$p_arr[0]);
	      my $p_hash = $p_acc_hash->{acc_data}[0];

	      $arr1[3] = q|PHGSEED|; #replace database
	      $arr1[4] = $p_arr[0]; #replace hit_name
                  #replace hit_description
	      $arr1[13] = (defined $p_hash->{desc}) ? $utils->trim($p_hash->{desc}) : 'UNKNOWN';

	      my $phgseed_row .= join("\t",@arr1)."\t".$taxonomy;

                  # if acc has meaning full fxn 1 then set fxnal hit at 1 else
	      if ((!$pFxn) && (defined $p_hash->{fxn1}) && ($p_hash->{fxn1} !~ /unknown|unclassified|unassigned|uncharacterized/i) && length($p_hash->{fxn1})){
		  $phgseed_row .= "\t1\n";
		  $pFxn=1;
	      } else { $phgseed_row .= "\t0\n"; }
	  ...
	  if(defined $u_hash->{aclame_acc} && (length($utils->trim($u_hash->{aclame_acc})) > 1) && ($u_hash->{aclame_acc} !~ /null/i)){
		  #reset array.
		  @arr1 = ();
		  @arr1 = split(/[\t]/,$seqline);
		  ...
		  # if acc has meaning full fxn 1 then set fxnal hit at 1 else 0
		  if ((!$aFxn) && (defined $a_hash->{fxn1}) && ($a_hash->{fxn1} !~ /unknown|unclassified|unassigned|uncharacterized/i) && length($a_hash->{fxn1})){
			$aclame_row .= "\t1\n";
			$aFxn=1;
		  } else { $aclame_row .= "\t0\n"; }
