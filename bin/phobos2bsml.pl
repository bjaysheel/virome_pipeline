...
phobos2bsml.pl - convert phobos output to BSML
...
B<--input,-i> 
    Input .out file from a Phobos search.
...
This script is used to convert the output from an phobos run into BSML.
...
while (<$ifh>) {
    ...
    if($_ !~ /^\#/) {
        ...
        if(defined($start) && defined($stop)) {
            ...
            $island->addBsmlLink('analysis', '#phobos_analysis', 'computed_by');
...
sub add_sequence {
    ...
    $seq->addBsmlLink('analysis', '#phobos_analysis', 'input_of');
