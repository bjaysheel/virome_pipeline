...
sub addBsmlFeatureTable {
    ...
    my ($table) = @_;
    if (!defined($table)){
	$table = new BSML::BsmlFeatureTable();
	if (!defined($table)){
	    die "Could not instantiate BSML::BsmlFeatureTable";
	}
    }

    push( @{$self->{'BsmlFeatureTables'}}, $table );

    my $index = @{$self->{'BsmlFeatureTables'}} - 1;
    return $index;
}

sub addBsmlFeatureTables {

    my $self = shift;
    my ($tables) = @_;
    if (!defined($tables)){
	die "tables was not defined";
    }

    my $tableCtr=0;
    foreach my $table (@{$tables}){
	$self->addBsmlFeatureTable($table);
	$tableCtr++;
    }

    print "Added '$tableCtr' BsmlFeatureTable objects to the BsmlSequence\n";
}
...
sub addBsmlFeatureGroup {
    ...
    if (!defined($group)){
	$group = new BSML::BsmlFeatureGroup();
	if (!defined($group)){
	    die "Could not instantiate BSML::BsmlFeatureGroup";
	}
    }

    push( @{$self->{'BsmlFeatureGroups'}}, $group );
...
sub addBsmlFeatureGroups {

    my $self = shift;
    my ($groups) = @_;

    if (!defined($groups)){
	die "groups was not defined";
    }

    my $groupCtr=0;

    foreach my $group (@{$groups}){
	$self->addBsmlFeatureGroup($group);
	$groupCtr++;
    }

    print "Added '$groupCtr' BsmlFeatureGroup objects to the BsmlSequence\n";
