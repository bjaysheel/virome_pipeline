...
sub addattr
  {
    ...
    if( defined( $value ) ){
        ...
        ## clean up attributes which reference ids
           ...
           my $clean_id = getCleanID($value);
           $value = $clean_id;
       } elsif (( $self->isa('BSML::BsmlAlignedSequence') && $key eq 'name') ||
		( $self->isa('BSML::BsmlSequenceData') && $key eq 'seq-name')) {
	   ## Only attempt to clean the value if processing either a <Aligned-sequence> name
	   ## attribute or <Sequence-data> seq-name attribute.
           my $clean_id = getCleanID($value);
           $value = $clean_id;
       }
...
sub hasBsmlLink {
    ...
    if ( exists $self->{'BsmlLink'} ) {
	if ( scalar ( @{$self->{'BsmlLink'}} ) > 0 ) {
	    return 1;
	}
    }
    return 0;
...
sub getBsmlLinkHrefByRel {
	...
	foreach my $link ( @{$self->{'BsmlLink'}} ){
		...
		if ($link->{'rel'} eq $rel ){
			...
			$logger->logdie("href does not exist for rel '$link->{'rel'}' for BSML Link element");
		    }
		}
	    }
	    else {
		$logger->logdie("rel does not exist for BSML Link element");
	    }
	}
	$logger->warn("No BsmlLink with rel '$rel' for the BSML Link element");		
    }
    else {
	$logger->warn("No BsmlLink for the BSML Link element");
    }
