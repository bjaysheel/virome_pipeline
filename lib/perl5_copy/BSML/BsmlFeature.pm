...
sub hasSiteLoc {
    ...
    if ( exists $self->{'BsmlSite-Loc'} ){
	if ( scalar(@{$self->{'BsmlSite-Loc'}}) > 0 ) {
	    return 1;
	}
    }
    return 0;
...
sub getFirstSiteLocSitePos {

    my $self = shift;

    if ( $self->hasSiteLoc()) {
	if ( exists $self->{'BsmlSite-Loc'}->[0]->{'sitepos'} ){
	    return $self->{'BsmlSite-Loc'}->[0]->{'sitepos'};
	}
	else {
	    $self->{_logger}->logdie("BsmlSite-Loc sitepos was not defined Feature with id '$self->returnattr('id')'");
	}
    }
    else {
	$self->{_logger}->logdie("No data in BsmlSite-Loc for Feature with id '$self->returnattr('id')'");
    }
}

sub getFirstSiteLocComplement {

    my $self = shift;

    if ( $self->hasSiteLoc()) {
	if ( exists $self->{'BsmlSite-Loc'}->[0]->{'complement'} ) {
	    return $self->{'BsmlSite-Loc'}->[0]->{'complement'};
	}
	else {
	    $self->{_logger}->logdie("BsmlSite-Loc complement was not defined for Feature with id '$self->returnattr('id')'");
	}
    }
    else {
	$self->{_logger}->warn("No data in BsmlSite-Loc for Feature with id '$self->returnattr('id')'");
    }
}
...
sub hasIntervalLoc {
    ...
    if ( exists $self->{'BsmlInterval-Loc'} ){
	if ( scalar(@{$self->{'BsmlInterval-Loc'}}) > 0 ) {
	    return 1;
	}
    }
    return 0;
...
sub getFirstIntervalLocStartPos {

    my $self = shift;

    if ( $self->hasIntervalLoc()) {
	if ( exists $self->{'BsmlInterval-Loc'}->[0]->{'startpos'}){
	    return $self->{'BsmlInterval-Loc'}->[0]->{'startpos'};
	}
	else {
	    $self->{_logger}->logdie("BsmlInterval-Loc startpos was not defined for Feature with id '$self->returnattr('id')'");
	}
    }
    else {
	$self->{_logger}->warn("No data in BsmlInterval-Loc for Feature with id '$self->returnattr('id')'");
    }
}

sub getFirstIntervalLocEndPos {

    my $self = shift;

    if ( $self->hasIntervalLoc()) {
	if ( exists $self->{'BsmlInterval-Loc'}->[0]->{'endpos'}){
	    return $self->{'BsmlInterval-Loc'}->[0]->{'endpos'};
	}
	else {
	    $self->{_logger}->logdie("BsmlInterval-Loc endpos was not defined for Feature with id '$self->returnattr('id')'");
	}
    }
    else {
	$self->{_logger}->warn("No data in BsmlInterval-Loc for Feature with id '$self->returnattr('id')'");
...
sub getFirstIntervalLocComplement {

    my $self = shift;

    if ( $self->hasIntervalLoc()) {
	if ( exists $self->{'BsmlInterval-Loc'}->[0]->{'complement'}){
	    return $self->{'BsmlInterval-Loc'}->[0]->{'complement'};
	}
	else {
	    $self->{_logger}->logdie("BsmlInterval-Loc complement was not defined for Feature with id '$self->returnattr('id')'");
	}
    }
    else {
	$self->{_logger}->warn("No data in BsmlInterval-Loc for Feature with id '$self->returnattr('id')'");
