...
# 
#
# Purpose:        To migrate the prok/eukaryotic organisms from legacy prok/euk database
#                 schema to BSML documents
#
#
# Note that the Sequence Ontology term 'polypeptide' replaces 'protein' (bug 2148) 
#
#---------------------------------------------------------------------------------------
=head1 NAME
...
#-------------------------------------------------------------
my ($database, $username, $password, $debug_level, $help, $man, $log4perl, $asmbl_id, $fastadir,
    ...
    $noDieNullAsmFeatureSequence, $idgen_identifier_version, $sourcename); 
...
if (!defined($idgen_identifier_version)){
    ...
    if ($idgen_identifier_version !~ /^\d+$/){
	$logger->logdie("idgen_identifier_version '$idgen_identifier_version' must be a positive integer");
    }
}
...
my $validSchemaTypes = { 'prok' => 1,
			 'ntprok' => 1,
			 'euk' => 1 };

## These are the only valid mode types
my $validModeTypes = { '1' => 1,   # migrate annotation only
...
my $schemaTypeToConf = { 'euk' => "Euk:Sybase:SYBTIGR",
			 'ntprok' => "Prok:Sybase:SYBTIGR",
			 'prok' => "Prok:Sybase:SYBTIGR" };

## The legacy annotation feat_types are mapped
## to specific SO types
my $classMappingLookup = { 'NTORF' => 'NTORF',
			   'ORF'   => 'ORF',
			   'tRNA'  => 'tRNA',
			   'snRNA'  => 'snRNA',
			   'sRNA'  => 'snRNA',
...
## <Sequence> element objects.
my $ntprokSequenceTypes = { 'NTORF' => 1,
			    'TERM' => 1,
			    'RBS' => 1,
			    'PMARK' => 1 };

## It is okay for asm_feature.sequence == null where feat_type are the following:
my $noDieNullAsmFeatureSequenceLookup = { 'PMARK' => 1 };

##
my $schemaTypeToSequenceTypes = { 'euk' => $eukSequenceTypes,
				  'prok' => $prokSequenceTypes,
				  'ntprok' => $ntprokSequenceTypes };

##
my $accessionDatabaseLookup = { 'PID' => 'NCBI_gi',
				'protein_id' => 'Genbank',
				'SP' => 'Swiss-Prot',
				'ECOCYC' => 'Ecocyc' };

##
my $rnaFeatureTypes = { 'tRNA' => 1,
			'rRNA' => 1,
			'sRNA' => 1,
			'ncRNA' => 1 };

##
my $ntprokRnaFeatureTypes = { 'NTtRNA' => 1,
			      'NTrRNA' => 1,
			      'NTmisc_RNA' => 1 };

my $schemaTypeToRnaFeatureTypes = { 'euk' => $rnaFeatureTypes,
				    'prok' => $rnaFeatureTypes,
				    'ntprok' => $ntprokRnaFeatureTypes };


##
my $peptideScoreTypeLookup = { 'Y-score' => 'y-score',
...
##
my $identXrefTypeLookup = { 'product name' => 'gene_product_name',
			    'gene name' => 'gene_name',
			    'gene symbol' => 'gene_symbol',
			    'genbank accession' => 'genbank' };

##
my $schemaTypeToGoRoleFeatureType = { 'euk' => 'TU',
				      'prok' => 'ORF',
				      'ntprok' => 'NTORF' };


##
my $berScoreTypeLookup = { 'Pvalue' => 'runprob',
			   'score' => 'runscore',
...
##
my $hmm2ScoreTypeLookup = { 'e-value' => 'runprob' ,
			    'score' => 'runscore' };

## 
my $prositeScoreTypeLookup = { 'hit' => 'residues' };

my $computeScoreTypeLookup = { 'HMM2' => $hmm2ScoreTypeLookup,
			       'PROSITE' => $prositeScoreTypeLookup,
			       'BER' => $berScoreTypeLookup };

##
my $prokSchemaTypeToFeature = {'prok' => 'ORF',
			       'ntprok' => 'NTORF' };

##
my $prokOrfAttributeAttTypes = { 'MW' => 'MW',
				 'PI' => 'pI',
				 'OMP' => 'outer_membrane_protein' };

##
my $legacyAnnoAttTypes = { 'coords' => 'transmembrane_coords',
			   'regions' => 'transmembrane_regions',
			   'OMP' => 'outer_membrane_protein' };


my $prokCdsOrfAttributeAttTypes = { 'GC' => 'percent_GC' };


my $berEvTypeLookup = { 'BER' => 1,
			'AUTO-BER' => 1 };

my $hmm2EvTypeLookup = { 'HMM2' => 1 };

my $cogEvTypeLookup = { 'COG accession' => 1 };

my $prositeEvTypeLookup = { 'PROSITE' => 'PROSITE' };

my $computeEvTypeLookup = { 'HMM2' => $hmm2EvTypeLookup,
			    'BER' => $berEvTypeLookup,
			    'COG' => $cogEvTypeLookup,
			    'PROSITE' => $prositeEvTypeLookup};

my $cloneInfoTypes = { 'assembly_status' => 1,
#		       'length' => 1,
		       'is_final' => 1,
		       'fa_left' => 1,
		       'fa_right' => 1,
...
my $rbsAnalysisAttributeLookup = { 'version' => 'RBS_version',
				   'program' => 'RBS',
				   'name' => 'RBS_analysis',
				   'method' => 'RBS',
				   'sourcename' => $sourcename };

my $termAnalysisAttributeLookup = { 'version' => 'TERM_version',
				    'program' => 'TERM',
				    'name' => 'TERM_analysis',
				    'method' => 'TERM',
				    'sourcename' => $sourcename };

my $hmm2AnalysisAttributeLookup = { 'version' => 'HMM2_version',
				    'program' => 'HMM2',
				    'name' => 'HMM2_analysis',
				    'sourcename' => $sourcename };

my $ber2AnalysisAttributeLookup = { 'version' => 'BER_version',
				    'program' => 'BER',
				    'name' => 'BER_analysis',
				    'sourcename' => $sourcename };

my $ncbiCogAnalysisAttributeLookup = { 'version' => 'legacy',
				       'program' => 'NCBI_COG',
				       'name' => 'NCBI_COG',
				       'sourcename' => $sourcename };

my $prositeAnalysisAttributeLookup = { 'version' => 'legacy',
				       'program' => 'PROSITE',
				       'name' => 'PROSITE',
				       'sourcename' => $sourcename };

my $interproAnalysisAttributeLookup = { 'version' => 'legacy',
					'program' => 'INTERPRO',
					'name' => 'INTERPRO',
					'sourcename' => $sourcename };

my $repeatMaskerAnalysisAttributeLookup = { 'version' => 'legacy',
...
my $analysisAttributeLookup = { 'RBS' => $rbsAnalysisAttributeLookup,
...
my $computeDatabaseLookup = { 'PFAM' => 'Pfam',
			      'PF' => 'Pfam',
			      'COG' => 'COG_Cluster',
			      'PS' => 'PROSITE',
			      'TIGR' => 'TIGR_TIGRFAMS',
...
my $terminatorAttributeLookup = { 'direction' => 'term_direction',
				  'confidence' => 'term_confidence' };

my $seqPairRunAttributeTypes = { 'refpos' => 1,
				 'runlength' => 1,
				 'refcomplement' => 1,
				 'comppos' => 1,
				 'comprunlength' => 1,
...
my $featTypeToMoleculeTypeLookup = { 'assembly' => 'dna',
				     'polypeptide' => 'aa',
				     'CDS' => 'dna',
				     'tRNA' => 'dna',
				     'rRNA' => 'dna',
...
my $eukOrfAttTypeLookup = { 'SP-HMM' => 'signalP_curated',
			    'targetP' => 'targetP_curated'};


my $storeSequenceAsSeqData = { 'pmark_spacer' => 1 };

my $withEvToEvidenceLookup = { 'SwissProt' => 'GO_REF:0000012',
...
if ( ! exists $validSchemaTypes->{$schema_type} ) {
    $logger->logdie("Invalid schema type '$schema_type'");
}
...
#
# Verify the genefinder flags
#
($exclude_genefinder, $include_genefinder) = &verify_and_set_genefinder_flags($exclude_genefinder, $include_genefinder);
...
print STDERR "Warning any existing bsml documents and multi-fasta files will be saved as .bak\n" if (defined($backup));
...
#
# Retrieve secondary type information from the $sequence_type
#
...
if (!defined($outputIdMappingFile)){
    ...
    $logger->warn("--output_id_mapping_file was not specified.  Setting default '$outputIdMappingFile'");
}

if (!defined($inputIdMappingDirectories)){
    $inputIdMappingDirectories = $outdir;
    $logger->warn("--input_id_mapping_directories was not specified.  Setting default '$inputIdMappingDirectories'. ".
		  "All files with extension '.idmap' will be read and their mappings will be loaded into the ID mapping lookup.");
}
...
if ((defined($model_list_file)) && ($model_list_file ne 'none')){
    ...
    $qualified_model_lookup = &create_qualified_tu_lookup($model_list_file);
...
if ((defined($tu_list_file)) && ($tu_list_file ne 'none')){
    ...
    $qualified_tu_lookup = &create_qualified_tu_lookup($tu_list_file);
}
else {
    ## Prism API support
    $tu_list_file = undef;
...
my $featNameToSpeciesLookup={};
my $featNameToComNameLookup={};
...
foreach my $assembly_id ( sort  @{$asmbl_list} ) {
	...
	if ( $schema_type eq 'euk' ) {
	    ...
	    $prism->loadFeatNameToSpeciesLookup($featNameToSpeciesLookup);
	    $prism->loadNtGeneProductNameLookup($featNameToComNameLookup);
    ...
    &verifyCDSAndExonStrands($gene_model_hash);
	...
	if ( $schema_type eq 'euk' ){
	    ...
	    &processLipoMembraneProteinLookup($polypeptide_feature_hash, $lipoMembraneProteinLookup, $doc);
...
sub create_euk_sequence_lookup {
	...
	my $ret = $prism->sequence_features($asmbl_id, $database, $feat_type);
...
sub createLipoMembraneProteinLookup {
   ...
   foreach my $arrayRef ( @{$lipoMembraneProteins} ){
	   ...
	   my $id = &get_uniquename($prism,
				    $idatabase,
				    $asmbl_id,
				    &cleanse_uniquename($arrayRef->[0]),
				    'polypeptide');
	   
	   $lipoMembraneProteinLookup->{$id} = $arrayRef->[1];
       }
   }
}
...
sub create_hmm2_evidence_lookup {
	...
	foreach my $scoreType ( keys %{$hmm2ScoreTypeLookup}){
		...
		if ( exists $localScoreTypeLookup->{$scoreType} ) {
		    ...
		    my ($comppos, $comprunlength, $compcomplement) = &compute_orientation($block->[6], $block->[7]); 

		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{$hmm2ScoreTypeLookup->{$scoreType}} = $block->[3];
		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{'refpos'} = $refpos;
		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{'runlength'} = $runlength;
		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{'refcomplement'} = $refcomplement;
		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{'comppos'} = $comppos;
		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{'comprunlength'} = $comprunlength;
		    $v->{$asmbl_id}->{$evType}->{$block->[1]}->{$block->[2]}->{$key}->{'compcomplement'} = $compcomplement;
...
sub create_cog_evidence_lookup {
   ...
   foreach my $evType ( keys %{$localEvTypeLookup} ) {
	   ...
	   my ($refpos, $runlength, $refcomplement) = &compute_orientation($block->[3], $block->[4]); 
	   
	   my ($comppos, $comprunlength, $compcomplement) = &compute_orientation($block->[5], $block->[6]); 

	   my $tmpLookup = { 'refpos' => $refpos,
			     'runlength' => $runlength,
			     'refcomplement' => $refcomplement,
			     'comppos' => $comppos,
			     'comprunlength' => $comprunlength,
			     'compcomplement' => $compcomplement };
...
sub create_prosite_evidence_lookup {
   ...
   foreach my $evType ( keys %{$localEvTypeLookup} ){
	   ...
	   foreach my $block ( @{$ret} ){
	       ...
	       my $refcomplement = ($block->[4] > $block->[5]) ? 1: 0;

	       my $compcomplement = ($block->[6] > $block->[7]) ? 1 : 0;
...
sub retrieve_organism_data {
	...
	if ($orgdata->{'genetic_code'} !~ /^\d+$/){
	    if (($orgdata->{'genetic_code'} eq 'microbial') || ($orgdata->{'genetic_code'} eq 'nt-microbial')){
...
sub create_assembly_sequence_component {
	...
	if (!defined($attribute_elem)){
	    $logger->logdie("Could not create <Attribute> for the '$asmbl_uniquename' assembly's molecule_name '$molecule_name_organism'");
	}
    }
...
sub store_sequence_elements {
	...
	if (  exists $storeSequenceAsSeqData->{$class}){
	    ...
	    my $tmpArray = [$seqfeature_id, $sequence_subfeature_hash->{'sequence'}];
	    
	    push ( @{$fastasequences->{$asmbl_id}->{$class}}, $tmpArray);
...
sub store_prok_gene_model_subfeatures {
	...
	if ($class eq 'transcript'){
	    ...
	    &addBsmlAttributeForClostoxByAttributeName($doc,
						       $feature_elem,
						       $orfhash->{'feat_name'},
						       'nt_gene_product_name',
						       $featNameToComNameLookup);
...
sub store_euk_gene_model_subfeatures {
	...
	if ((exists $coding_regions->{$tu_feat_name}) && (defined($coding_regions->{$tu_feat_name}))){
		...
		if (  (!defined($model->{'sequence'})) && ($transcripts->{$tu_feat_name}->{'is_pseudogene'} != 1)) {
		    $logger->logdie("model '$model->{'feat_name'}' had no asm_feature.sequence and is not part of a pseudogene");
		}
...
sub get_asmbl_id_list_from_file {
    ...
    ## If user invokes --asmbl_file, and if secondary sequence-types were 
    ## defined in the file, need to pass that information forward (bug 2086).
    open (ASMBLFILE, "<$asmbl_file") or $logger->logdie("Could not open file '$asmbl_file' in read mode");

    my @list;
    my $secondaryhash = {};

    while (my $line = <ASMBLFILE>){
	...
	if (defined($secondary)){
	    ...
	    my ($db, $type) = split(/:/, $secondary);

	    $secondaryhash->{$asmbl_id} = { 'ontology' => $db,
					    'seqtype'  => $type
					};
    ...
    my $comma = ",";
    my $string = join ($comma, @list);

    return ($string, $secondaryhash);
	...
	if (( exists $polypeptide_feature_hash->{$polypeptide}) && (defined($polypeptide_feature_hash->{$polypeptide})) ) {
		...
		if ( scalar(@{$orf_attributes_hash->{$polypeptide}->{'attributes'}}) > 0 ) {
			...
			if (( exists $attribute->{'content'}) && (defined($attribute->{'content'})) ) {
			    ...
			    $logger->logdie("Could not create <Attribute> element object reference for orf_attribute '$attribute' value '$attribute->{$attribute}' for polypeptide '$polypeptide'") if (!defined($attribute_elem));
			}
			else {
			    $logger->logdie("The content for this attribute name '$attribute->{'name'}' polypeptide '$polypeptide' was not defined");
			}
		    }
		}
		else {
...
#------------------------------------------------------------------------------
# processLipoMembraneProteinLookup()
#
#------------------------------------------------------------------------------
sub processLipoMembraneProteinLookup {

    my ($polypeptideBsmlFeatureLookup, $lipoMembraneProteinLookup, $doc) = @_;

    foreach my $feat_name ( keys %{$lipoMembraneProteinLookup}){
	
	if (( exists $polypeptideBsmlFeatureLookup->{$feat_name} ) && 
	    (defined($polypeptideBsmlFeatureLookup->{$feat_name} )) ) {
	    
	    my $value = $lipoMembraneProteinLookup->{$feat_name};
	    
	    ## Strip trailing whitespaces
	    $value =~ s/\s+$//;

	    my $attributeBsmlElement = $doc->createAndAddBsmlAttribute( $polypeptideBsmlFeatureLookup->{$feat_name}, # elem
									'lipo_membrane_protein',    # key
									$value                      # value
									);

	    if (!defined($attributeBsmlElement)){
		$logger->logdie("Could not create <Attribute> element object with name 'lipo_membrane_protein' ".
				"content '$value' for ORF with feat_name '$feat_name'");
	    }
	}
	else {
	    $logger->warn("While processing lipo_membrane_protein data, encountered a polypeptide ".
			  "with uniquename '$feat_name'.  This polypeptide does not exist in the ".
			  "polypeptideBsmlFeatureLookup hash lookup.  Could be that this polypeptide ".
			  "did not have any sequence associated with it.  Please investigate.");
	}
    }
}
...
sub create_repeat_feature_lookup {
	...
	for (my $i=0 ; $i < scalar(@{$ret}); $i++) {
	    ...
	    push ( @{$repeat->{$asmbl_id}}, [$ret->[$i][1],  # asm_feature.feat_name
					     $ret->[$i][2],  # asm_feature.end5
					     $ret->[$i][3],  # asm_feature.end3
					     $ret->[$i][4],  # ORF_attribute.score
					     ]);
	}
...
#---------------------------------------------------------
# create_qualified_tu_lookup()
#
#---------------------------------------------------------
sub create_qualified_tu_lookup {
	...
	if (-f $file){
	    if (-r $file){
		if (!-z $file){
		    open (INFILE, "<$file") or $logger->logdie("Could not open file '$file' for input: $!");
			...
			if (!exists $qualified_tu_lookup->{$line}){
			    $qualified_tu_lookup->{$line}++;
			}
			else {
		    ...
		    $logger->logdie("$file' contains zero content");
		}
	    }
	    else {
	    ...
	    $logger->logdie("'$file' is not a regular file");
	}
    }
    else {
...
sub createInterproEvidenceLookup {
   ...
   foreach my $block ( @{$ret} ){
       ...
       my $refcomplement = ($block->[2] > $block->[3]) ? 1 : 0;
       
       my $compcomplement = ($block->[2] > $block->[3]) ? 1 : 0;
...
sub check_id_repository {
	...
	if (-e $idRepository){
		...
		if (-r $idRepository){
		    ...
		    $logger->warn("id_repository '$idRepository' does not have read permissions.  Using default ENV{ID_REPOSITORY} '$ENV{ID_REPOSITORY}'");
		}
	    }
	    else{
		$logger->warn("id_repository '$idRepository' does not have write permissions.  Using default ENV{ID_REPOSITORY} '$ENV{ID_REPOSITORY}'");
	    }
	}
	else{
	    $logger->warn("id_repository '$idRepository' does not exist.  Using default ENV{ID_REPOSITORY} '$ENV{ID_REPOSITORY}'");
	}
    }
    else {
	$logger->warn("id_repository '$idRepository' was not defined.  Using default ENV{ID_REPOSITORY} '$ENV{ID_REPOSITORY}'");
    }
}
...
sub verifySequenceFeaturePairs {
    my ($seqLookup, $rnaLookup, $class) = @_;

    print "Checking Sequence Feature pair completeness for class '$class'\n";

    my $missingLookup = {};
    my $missingCtr=0;

    foreach my $asmbl_id (sort keys %{$seqLookup}){
	foreach my $rnaHash ( @{$rnaLookup->{$asmbl_id}}){
	    my $rnaFeatName = $rnaHash->{'feat_name'};
	    my $found=0;
	    foreach my $seqHash ( @{$seqLookup->{$asmbl_id}}){
		if (( exists $seqHash->{'feat_type'} ) &&
		    ( $seqHash->{'feat_type'} eq '$class') && 
		    ( exists $seqHash->{'feat_name'} ) &&
		    ( $seqHash->{'feat_name'} eq $rnaFeatName )){
		    $found=1;
		    last;
		}
	    }
	    if ($found==0){
		$missingCtr++;
		$missingLookup->{$rnaFeatName}++;
	    }
	}
    }

    if ($missingCtr>0){
	$logger->fatal("The following '$missingCtr' Features with class '$class' were missing");
	foreach my $id (sort keys %{$missingLookup}){
	    $logger->fatal("$id");
	}
	$logger->logdie("Please see log file");
    }
}

sub addBsmlAttributeForClostoxByAttributeName {

    my ($bsmlBuilder, $bsmlFeature, $feat_name, $name, $lookup) = @_;

    ## clostox specific hack
    if (! exists $lookup->{$feat_name} ){
	$logger->warn("feat_name '$feat_name' does not exist in the lookup");
    } else {
	my $value = $lookup->{$feat_name};
	
	my $bsmlAttribute = $bsmlBuilder->createAndAddBsmlAttribute($bsmlFeature, # <Feature> element object reference
								    $name,  # name
								    $value  # content
								    );
	    
	if (!defined($bsmlAttribute)){
	    $logger->logdie("Could not create <Attribute> element object with ".
			    "name '$name' content '$value' for the following BSML ".
			    "<Feature> object:" . Dumper $bsmlFeature);
	}
    }
}
