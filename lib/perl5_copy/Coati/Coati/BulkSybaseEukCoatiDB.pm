package Coati::Coati::BulkSybaseEukCoatiDB;

# $Id: BulkSybaseEukCoatiDB.pm,v 1.4 2003-12-04 15:30:40 angiuoli Exp $

# Copyright (c) 2002, The Institute for Genomic Research. All rights reserved.

=head1 NAME

BulkSybaseEukCoatiDB.pm - One line summary of purpose of class (or file).

=head1 VERSION

This document refers to version N.NN of BulkSybaseEukCoatiDB.pm, released MMMM, DD, YYYY.
...
use strict;
use base qw(Coati::BulkSybaseHelper Coati::Coati::SybaseEukCoatiDB);

sub test_BulkSybaseEukCoatiDB {
    my ($self, @args) = @_;
    $self->{_logger}->info("Hello from :",__PACKAGE__,"!!!\n");
    return $self->test_SybaseEukCoatiDB();
}

sub testDB {
    my ($self, @args) = @_;
    $self->{_logger}->info("Hello from :",__PACKAGE__,"!!!\n");
    return $self->test_BulkSybaseEukCoatiDB;
}
