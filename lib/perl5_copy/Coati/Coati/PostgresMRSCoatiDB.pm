package Coati::Coati::PostgresMRSCoatiDB;

# $Id: PostgresMRSCoatiDB.pm,v 1.1 2005-11-11 15:20:58 angiuoli Exp $

# Copyright (c) 2002, The Institute for Genomic Research. All rights reserved.

=head1 NAME

PostgresMRSCoatiDB.pm - One line summary of purpose of class (or file).

=head1 VERSION

This document refers to version N.NN of PostgresMRSCoatiDB.pm, released MMMM, DD, YYYY.
...
use strict;
use base qw(Coati::Coati::MRSCoatiDB Coati::PostgresHelper);

sub test_PostgresMRSCoatiDB {
    my ($self, @args) = @_;
    $self->{_logger}->info("Hello from :",__PACKAGE__,"!!!\n");
    return $self->test_MRSCoatiDB();
}

sub testDB {
    my ($self, @args) = @_;
    $self->{_logger}->info("Hello from :",__PACKAGE__,"!!!\n");
    return $self->test_PostgresMRSCoatiDB();
}
