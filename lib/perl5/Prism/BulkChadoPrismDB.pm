package Prism::BulkChadoPrismDB;

use strict;
use base qw(Prism::ChadoPrismDB Coati::Coati::BulkChadoCoatiDB );
use Data::Dumper;

##
## All methods here are only for bulk loading support
## database type/vendor independent.
## Type/vendor specific support will be implemented in
## BulkSybaseChadoPrismDB.pm for Sybase or in
## PostgresChadoPrismDB.pm for PostgreSQL
##

our $limitsLookup = { 'tableinfo' => { 'name' => 30,
				       'primary_key_column' => 30 },
		      'project' => { 'name' => 255,
				     'description' => 255 },
		      'db' => { 'name' => 50,
				'description' => 255,
		      ...
		      'cvterm' => { 'name' => 255,
				    'definition' => 1200 },
		      'cvtermsynonym' => { 'synonym' => 1024 },
		      'cvtermprop' => { 'value' => 1000 },
		      'dbxrefprop' => { 'value' => 255 },
		      'organism' => { 'genus' => 100,
				      'species' => 100,
				      'abbreviation' => 50,
				      'common_name' => 150,
				      'comment' => 255 },
		      'organismprop' => { 'value' => 255 },
		      'pub' => { 'title' => 255,
				 'volumetitle' => 255, 
				 'volume' => 255,
				 'series_name' => 255,
		      ...
		      'feature' => { 'name' => 255,
				     'uniquename' => 255,
				     'md5checksum' => 32 },
		      'featureloc' => { 'residue_info' => 7500 },
		      'featureprop' => { 'value' => 2000 },
		      'feature_relationship' => { 'value' => 255 }, 
		      'feature_relationshipprop' => { 'value' => 255 },
		      'feature_cvtermprop' => { 'value' => 255 },
		      'synonym' => { 'name' => 255,
				     'synonym_sgml' => 255 },
		      'analysis' => { 'name' => 255,
				      'description' => 255,
				      ...
				      'sourceversion' => 50,
				      'sourceuri' => 255 },
		      'analysisprop' => { 'value' => 255 },
		      'phylotree' => { 'name' => 255,
				       'comment' => 255 },
		      'phylonode' => { 'label' => 255 },
		      'phylonodeprop' => { 'value' => 255 },
		      'infectionprop' => { 'value' => 255 },
		      'transmissionprop' => { 'value' => 255 },
		      'incidentprop' => { 'value' => 255 },
		      'contact' => { 'name' => 255,
				     'description' => 255 },
		      'contactprop' => { 'value' => 255 },
		      'genotype' => { 'name' => 255,
				      'uniquename' => 255,
				      'description' => 255 },
		      'environment' => { 'uniquename' => 255,
					 'description' => 255},
		      'phendesc' => { 'description' => 255 },
		      'stock' => { 'name' => 255,
				   'uniquename' => 255,
				   'description' => 255 },
		      'stockprop' => { 'value' => 255 },
		      'stock_relationship' => { 'value' => 255 },
		      'stockcollection' => { 'name' => 255,
					     'uniquename' => 255 },
		      'stockcollectionprop' => { 'value' => 255 }
		  };
				      


sub test_BulkChadoPrismDB {
    my ($self, @args) = @_;
...
sub testProjDB {
    my ($self, @args) = @_;
    $self->{_logger}->info("Hello from :",__PACKAGE__,"!!!\n");
    return $self->test_BulkChadoPrismDB();
...
sub doBulkLoadTable {
    my ($self) = @_;
    my $class = ref($self);
    if ($class eq 'BulkChadoPrismDB'){
	$self->{_logger}->logdie("Cannot use abstract method ${class}::doBulkLoadTable");
    }
}
...
sub doBulkDumpTable {
    ...
    my $class = ref($self);
    if ($class eq 'BulkChadoPrismDB'){
	$self->{_logger}->logdie("Cannot use abstract method ${class}::doBulkDumpTable");
    }
}
...
sub do_store_new_feature {
    ...
    $dbxref_id = ( defined $params{'dbxref_id'}) ? $params{'dbxref_id'} : '' ;

    $name = ( defined $params{'name'}) ? $params{'name'} : '' ;
    ...
    $seqlen = ( defined $params{'seqlen'}) ? $params{'seqlen'} : '' ;
...
sub do_store_new_featureloc{
    ...
    $srcfeature_id = (defined $params{'srcfeature_id'}) ? $params{'srcfeature_id'} : '';
    ...
    $fmax = (defined $params{'fmax'}) ? $params{'fmax'} : '';
    
    $is_fmax_partial = (defined $params{'is_fmax_partial'}) ? $params{'is_fmax_partial'} : 0;

    $strand = (defined $params{'strand'}) ? $params{'strand'} : '';

    $phase = (defined $params{'phase'}) ? $params{'phase'} : '';
	...
	if (!defined($featureloc_id)){
	    ...
	    $self->_add_row(
			    ...
			    $locgroup,         ##  numeric(9,0) DEFAULT '' NOT NULL
			    $rank              ##  numeric(9,0) DEFAULT '' NOT NULL
			    );
...
sub do_store_new_analysisfeature{
    ...
    $rawscore = (defined $params{'rawscore'}) ? $params{'rawscore'} : '';

    $normscore = (defined $params{'normscore'}) ? $params{'normscore'} : '';

    $significance = (defined $params{'significance'}) ? $params{'significance'} : '';

    $pidentity = (defined $params{'pidentity'}) ? $params{'pidentity'} : '';

    $type_id = (defined $params{'type_id'}) ? $params{'type_id'} : '';
...
sub do_store_new_cvtermpath {
    ...
    $pathdistance = (defined $params{'pathdistance'}) ?  $params{'pathdistance'} : '';
...
sub do_store_new_tableinfo {
    ...
    $view_on_table_id = (defined $params{'view_on_table_id'}) ?  $params{'view_on_table_id'} : '';

    $superclass_table_id = (defined $params{'superclass_table_id'}) ? $params{'superclass_table_id'} : '';
...
sub do_store_new_phylonode {
    ...
    $parent_phylonode_id = (defined $params{'parent_phylonode_id'}) ?  $params{'parent_phylonode_id'} : '';

    $feature_id = (defined $params{'feature_id'}) ?  $params{'feature_id'} : '';
    ...
    $distance = (defined $params{'distance'}) ?  $params{'distance'} : '';
...
##---------------------------------------------------------------------------------------------------------------
## doStoreRecordsInCmProteins()
##
##---------------------------------------------------------------------------------------------------------------
sub doStoreRecordsInCmProteins {

    my ($self, %params) = @_;
    
    $self->{_logger}->debug("Args: ",join(',',@_)) if $self->{_logger}->is_debug;

    if ( ! exists $params{'protein_id'}){
	$self->{_logger}->logdie("protein_id was not defined");
    }
    if ( ! exists $params{'organism_id'}){
	$self->{_logger}->logdie("organism_id was not defined");
    }
    if ( ! exists $params{'uniquename'}){
	$self->{_logger}->logdie("uniquename was not exists");
    }
    if ( ! exists $params{'cds_id'}){
	$self->{_logger}->logdie("cds_id was not defined");
    }
    if ( ! exists $params{'gene_id'}){
	$self->{_logger}->logdie("gene_id was not defined");
    }
    if ( ! exists $params{'transcript_id'}){
	$self->{_logger}->logdie("transcript_id was not defined");
    }
    if ( ! exists $params{'exon_count'}){
	$self->{_logger}->logdie("exon_count was not defined");
    }
    if ( ! exists $params{'accession1'}){
	$self->{_logger}->logdie("accession1 was not defined");
    }
    if ( ! exists $params{'accession2'}){
	$self->{_logger}->logdie("accession2 was not defined");
    }
    if ( ! exists $params{'gene_product_name'}){
	$self->{_logger}->logdie("gene_product_name was not defined");
    }
    if ( ! exists $params{'fmin'}){
	$self->{_logger}->logdie("fmin was not defined");
    }
    if ( ! exists $params{'fmax'}){
	$self->{_logger}->logdie("fmax was not defined");
    }
    if ( ! exists $params{'seqlen'}){
	$self->{_logger}->logdie("seqlen was not defined");
    }
    if ( ! exists $params{'strand'}){
	$self->{_logger}->logdie("strand was not defined");
    }
    if ( ! exists $params{'srcfeature_id'}){
	$self->{_logger}->logdie("srcfeature_id was not defined");
    }

    my $seed = "$params{'protein_id'}";
    
    my $cm_proteins_id = $self->{_id_manager}->lookupId("cm_proteins", $seed);
    
    if (!defined($cm_proteins_id)){
	$cm_proteins_id = $self->{_id_manager}->nextId("cm_proteins", $seed);
	
	if (!defined($cm_proteins_id)){
	    $self->{_logger}->error("Could not retrieve cm_proteins_id from Coati::IdManager ".
				    "lookup, nor could it be generated.  Seed was 'cm_proteins', ".
				    "'$seed'.  Cannot prepare record for insertion into the ".
				    "cm_proteins BCP file.");
	}
	else {
	    $self->_add_row(
			    "cm_proteins",
			    ...
			    $params{'accession1'},
			    $params{'accession2'},
			    $params{'gene_product_name'},
			    $params{'fmin'},
			    $params{'fmax'},
			    $params{'seqlen'},
			    $params{'strand'},
			    $params{'srcfeature_id'}
			    );
	}
    }
    else {
	$self->{_logger}->warn("A record with protein_id '$params{'protein_id'}' was already ".
			       "prepared for the cm_proteins BCP file.");
    }

    return $cm_proteins_id;
}



##---------------------------------------------------------------------------------------------------------------
## doStoreRecordsInCmClusters()
##
##---------------------------------------------------------------------------------------------------------------
sub doStoreRecordsInCmClusters {

    my ($self, %params) = @_;
    
    $self->{_logger}->debug("Args: ",join(',',@_)) if $self->{_logger}->is_debug;

    if ( ! exists $params{'cluster_id'}){
	$self->{_logger}->logdie("cluster_id was not defined");
    }
    if ( ! exists $params{'analysis_id'}){
	$self->{_logger}->logdie("analysis_id was not defined");
    }
    if ( ! exists $params{'num_members'}){
	$self->{_logger}->logdie("num_members was not defined");
    }
    if ( ! exists $params{'num_organisms'}){
	$self->{_logger}->logdie("num_organisms was not defined");
    }
    if ( ! exists $params{'percent_coverage'}){
	$self->{_logger}->logdie("percent_coverage was not defined");
    }
    if ( ! exists $params{'percent_coverage'}){
	$self->{_logger}->logdie("percent_coverage was not defined");
    }
    
    my $seed = "$params{'cluster_id'}:$params{'analysis_id'}";
    
    my $cm_clusters_id = $self->{_id_manager}->lookupId("cm_clusters", $seed);
    
    if (!defined($cm_clusters_id)){
	$cm_clusters_id = $self->{_id_manager}->nextId("cm_clusters", $seed);
	
	if (!defined($cm_clusters_id)){
	    $self->{_logger}->error("Could not retrieve cm_clusters_id from Coati::IdManager ".
				    "lookup, nor could it be generated.  Seed was 'cm_clusters', ".
				    "'$seed'.  Cannot prepare record for insertion into the cm_clusters BCP file.");
	}
	else {
	    $self->_add_row(
			    "cm_clusters",
			    ...
			    $params{'percent_coverage'},
			    $params{'percent_identity'}
			    );
	}
    }
    else {
	$self->{_logger}->warn("A record with cluster_id '$params{'cluster_id'}' analysis_id ".
			       "'$params{'analysis_id'}' was already prepared for the cm_clusters ".
			       "BCP file.");
    }


    return $cm_clusters_id;
}

##-------------------------------------------------------------------------
## doStoreRecordsInCmClusterMembers()
##
##-------------------------------------------------------------------------
sub doStoreRecordsInCmClusterMembers {
    ...
    if ( ! exists $params{'accession1'}){
	$self->{_logger}->logdie("accession1");
    }
    if ( ! exists $params{'accession2'}){
	$self->{_logger}->logdie("accession2 was not defined");
    }

    my $seed = "$params{'cluster_id'}:$params{'feature_id'}";
    
    my $cm_cluster_members_id = $self->{_id_manager}->lookupId("cm_cluster_members", $seed);
    
    if (!defined($cm_cluster_members_id)){
	$cm_cluster_members_id = $self->{_id_manager}->nextId("cm_cluster_members", $seed);
	
	if (!defined($cm_cluster_members_id)){
	    $self->{_logger}->error("Could not retrieve cm_cluster_members_id from Coati::IdManager ".
				    "lookup, nor could it be generated.  Seed was 'cm_cluster_members', ".
				    "'$seed'.  Cannot prepare record for insertion into the cm_cluster_members BCP file.");
	}
	else {
	    $self->_add_row(
			    "cm_cluster_members",
			    $cm_cluster_members_id,
			    $params{'cluster_id'},
			    $params{'feature_id'},
			    $params{'organism_id'},
			    $params{'uniquename'},
	...
	$self->{_logger}->warn("A record with cluster_id '$params{'cluster_id'}' feature_id ".
			       "'$params{'feature_id'}' was already prepared for the cm_cluster_members ".
			       "BCP file.");
    }
    
    return $cm_cluster_members_id;
}
...
sub adjustString {
    ...
    if (($field eq 'residues') && ($table eq 'feature')){
	if ($self->{_logger}->is_debug()){
	    $self->{_logger}->debug("No length enforcement on feature.residues");
	}
    }
    else {
	if (defined $limitsLookup->{$table}){
	    if (defined $limitsLookup->{$table}->{$field}){
		
		my $limit = $limitsLookup->{$table}->{$field};
		
		my $length = length($value);
		
		if ($length > $limit){
		    my $oldvalue = $value;
		    $value = substr($oldvalue,0,$limit-1);
		    
		    $self->{_logger}->warn("Acceptable length for table '$table' field '$field' is ".
					   "'$limit'.  The length was '$length'.  Had to truncate ".
					   "the value from '$oldvalue' ".
					   "to new value:\n'$value'");
		}
	    }
	    else {
		$self->{_logger}->logdie("field '$field' does not exist in ".
					 "limitsLookup for table '$table'");
	    }
	}
	else {
	    $self->{_logger}->logdie("table '$table' does not exist in limitsLookup");
	}
    }
...
##-------------------------------------------------------------------------
## doStoreRecordsInCmBlast()
##
##-------------------------------------------------------------------------
sub doStoreRecordsInCmBlast {

    my ($self, %params) = @_;
    
    $self->{_logger}->debug("Args: ",join(',',@_)) if $self->{_logger}->is_debug;

    if ( ! defined $params{'qfeature_id'}){
	$self->{_logger}->logdie("qfeature_id was not defined");
    }
    if ( ! defined $params{'qorganism_id'}){
	$self->{_logger}->logdie("qorganism_id was not defined");
    }
    if ( ! defined $params{'hfeature_id'}){
	$self->{_logger}->logdie("hfeature_id was not defined");
    }
    if ( ! defined $params{'horganism_id'}){
	$self->{_logger}->logdie("horganism_id was not defined");
    }
    if ( ! defined $params{'per_id'}){
	$self->{_logger}->logdie("per_id");
    }
    if ( ! defined $params{'per_sim'}){
	$self->{_logger}->logdie("per_sim was not defined");
    }
    if ( ! defined $params{'p_value'}){
	$self->{_logger}->logdie("p_value was not defined");
    }
    if ( ! defined $params{'mfeature_id'}){
	$self->{_logger}->logdie("mfeature_id was not defined");
    }

    my $seed = "$params{'qorganism_id'}:$params{'horganism_id'}:$params{'qfeature_id'}:$params{'hfeature_id'}:$params{'mfeature_id'}";
    
    my $cm_blast_id = $self->{_id_manager}->lookupId("cm_blast", $seed);
    
    if (!defined($cm_blast_id)){
	$cm_blast_id = $self->{_id_manager}->nextId("cm_blast", $seed);
	
	if (!defined($cm_blast_id)){
	    $self->{_logger}->error("Could not retrieve cm_blast_id from Coati::IdManager ".
				    "lookup, nor could it be generated.  Seed was 'cm_blast', ".
				    "'$seed'.  Cannot prepare record for insertion into the cm_blast BCP file.");
	}
	else {
	    $self->_add_row(
			    "cm_blast",
	...
	$self->{_logger}->warn("A record with qorganism_id '$params{'qorganism_id'}' ".
			       "horganism_id '$params{'horganism_id'}' qfeature_id ".
			       "'$params{'qfeature_id'}' hfeature_id '$params{'hfeature_id'}' ".
			       "mfeature_id '$params{'mfeature_id'}' ".
			       "was already prepared for ".
			       "the cm_blast BCP file.");
    }
...
sub doStoreNewCmLineage {
    ...
    if ( ! defined $params{'num_members'}){
	$self->{_logger}->logdie("num_members was not defined");
    }
    if ( ! defined $params{'protein_id'}){
	$self->{_logger}->logdie("protein_id was not defined");
    }
    if ( ! defined $params{'num_orthologs'}){
	$self->{_logger}->logdie("num_orthologs was not defined");
    }
    if ( ! defined $params{'pub_locus'}){
	$self->{_logger}->logdie("pub_locus was not defined");
    }
    if ( ! defined $params{'gene_product_name'}){
	$self->{_logger}->logdie("gene_product_name was not defined");
    }
    if ( ! defined $params{'gene_length'}){
	$self->{_logger}->logdie("gene_length was not defined");
    }
    if ( ! defined $params{'protein_length'}){
	$self->{_logger}->logdie("protein_length was not defined");
    }
    if ( ! defined $params{'num_exons'}){
	$self->{_logger}->logdie("num_exons was not defined");
    }
    if ( ! defined $params{'num_paralogs'}){
	$self->{_logger}->logdie("num_paralogs was not defined");
    }
    if ( ! defined $params{'url'}){
	$self->{_logger}->logdie("url was not defined");
    }


    my $seed = "$params{'cluster_id'}:$params{'protein_id'}";
    
    my $cm_lineage_id = $self->{_id_manager}->lookupId("cm_lineage", $seed);
    
    if (!defined($cm_lineage_id)){
	$cm_lineage_id = $self->{_id_manager}->nextId("cm_lineage", $seed);
	
	if (!defined($cm_lineage_id)){
	    $self->{_logger}->error("Could not retrieve cm_blast_id from Coati::IdManager ".
				    "lookup, nor could it be generated.  Seed was 'cm_lineage', ".
				    "'$seed'.  Cannot prepare record for insertion into the cm_lineage BCP file.");
	}
	else {
	    $self->_add_row(
			    "cm_lineage",
			    ...
			    $params{'num_members'},
			    $params{'protein_id'},
			    $params{'num_orthologs'},
			    $params{'pub_locus'},
			    $params{'gene_product_name'},
			    $params{'gene_length'},
			    $params{'protein_length'},
			    $params{'num_exons'},
			    $params{'num_paralogs'},
			    $params{'url'},
			    );
	...
	$self->{_logger}->warn("A record with cluster_id '$params{'cluster_id'}' ".
			       "num_members '$params{'num_members'}' protein_id ".
			       "'$params{'protein_id'}' ".
			       "num_orthologs '$params{'num_orthologs'}' ".
			       "pub_locus '$params{'pub_locus'}' ".
			       "gene_product_name '$params{'gene_product_name'}'".
			       "gene_length '$params{'gene_length'}' ".
			       "protein_length '$params{'protein_length'}' ".
			       "num_exons '$params{'num_exon'}' ".
			       "num_paralogs '$params{'num_paralogs'}' ".
			       "url '$params{'url'}' ".
			       "was already prepared for ".
			       "the cm_lineage BCP file.");
    }
    
    return $cm_lineage_id;
}
