# $Id: Foundation.pm,v 1.16 2003/01/06 17:31:35 dkosack Exp $
...
=cut
   ...
   our $REVISION = (qw$Revision: 1.16 $)[-1];
   our $VERSION = '1.3'; 
...
=cut
   ...
   sub new() {
      ...
      my $prog_name  = $self->getProgramInfo('name');
      my $prog_invocation  = $self->getProgramInfo('invocation');
      ...
      if((defined $user_name) && (defined $host_name) && 
         (defined $prog_name) && (defined $prog_invocation)) {
         # Log program invocation
         $self->logLocal("START: $prog_name $prog_invocation", 0);
         $self->logLocal("Username: $user_name", 0);
         $self->logLocal("Hostname: $host_name", 0);
...
An example script using this module follows:
   ...
   MAIN: 
   {
      ...
      my $VERSION = '1.0';
      my $REVISION = (qw$Revision: 1.16 $)[-1];
