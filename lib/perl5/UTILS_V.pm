...
sub phgseed_lookup{
    my $self = shift;
    if (@_ > 0) {
	$self->{phgseed_lookup} = $_[0];
    }
    else {
	return $self->{phgseed_lookup};
    }
}
...
sub set_db_params{
	...
	$self->{db_user} = q|dnasko|;
        ...
        $self->{db_name} = q|virome_processing_1|;
        $self->{db_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_name} = q|virome|;
        $self->{u_name} = q|uniref_lookup|;
    }elsif ($env =~ /diag2/i){
	$self->{db_user} = q|dnasko|;
        $self->{db_pass} = q|dnas_76|;
        $self->{db_name} = q|virome_processing_2|;
        $self->{db_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_name} = q|virome|;
        $self->{u_name} = q|uniref_lookup|;
	...
	$self->{db_user} = q|dnasko|;
        $self->{db_pass} = q|dnas_76|;
        $self->{db_name} = q|virome_processing_3|;
        $self->{db_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_name} = q|virome|;
        $self->{u_name} = q|uniref_lookup|;
	...
	$self->{db_user} = q|dnasko|;
        $self->{db_pass} = q|dnas_76|;
        $self->{db_name} = q|virome_processing_4|;
        $self->{db_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_name} = q|virome|;
        $self->{u_name} = q|uniref_lookup|;
	...
	$self->{db_user} = q|dnasko|;
        $self->{db_pass} = q|dnas_76|;
        $self->{db_name} = q|virome_processing_5|;
        $self->{db_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_host} = q|dnode001.igs.umaryland.edu|;
        $self->{v_name} = q|virome|;
        $self->{u_name} = q|uniref_lookup|;
    ...
    $self->{db_live_user} = q|dnasko|;
    $self->{db_live_pass} = q|dnas_76|;
    $self->{db_live_name} = q|virome|;
    $self->{db_live_host} = q|192.168.125.36|;
}
...
sub db_live_user{
    my $self=shift;
    if (@_ > 0){
        $self->{db_live_user} = $_[0];
    } else {
        return $self->{db_live_user};
    }
}
sub db_live_pass{
    my $self=shift;
    if (@_ > 0){
        $self->{db_live_pass} = $_[0];
    } else {
        return $self->{db_live_pass};
    }
}
sub db_live_name{
    my $self=shift;
    if (@_ > 0){
        $self->{db_live_name} = $_[0];
    } else {
        return $self->{db_live_name};
    }
}
sub db_live_host{
    my $self=shift;
    if (@_ > 0){
        $self->{db_live_host} = $_[0];
    } else {
        return $self->{db_live_host};
    }
}
