...
=item $builder->addSequence( $sequence )

B<Description:> Add BsmlSequence to the BsmlDoc

B<Parameters:> $sequence (reference to BsmlSequence object)

B<Returns:> reference to the BsmlSequence
...
sub addSequence {
    ...
    my ($sequence) = @_;
    if (!defined($sequence)){
	die "sequence was not defined";
    }

    my $id = $sequence->returnattr('id');
    if (!defined($id)){
	die("id was not defined!");
    }

    BSML::BsmlDoc::BsmlSetDocumentLookup( $id, $sequence );

    return $self->returnBsmlSequenceR( $self->addBsmlSequence($sequence) );
}
...
sub createAndAddFeatureWithLoc
  {
    ...
    if (!defined($feature)){
	die "<Feature> BSML element object was not defined ";
    }
...
#
#  an 'id' argument is used as a standard attribute <Analysis id='foo'>
#   all other arguments are written as child Attribute elements
...
=item $builder->addGenome( $genome )

B<Description:> Add BsmlGenome to the BsmlDoc

B<Parameters:> $genome (reference to BsmlGenome object)

B<Returns:> reference to the BsmlGenome
...
sub addGenome {
    ...
    my ($genome) = @_;
    if (!defined($genome)){
	die "genome was not defined";
    }

    return $self->returnBsmlGenomeR( $self->addBsmlGenome($genome) );
}
