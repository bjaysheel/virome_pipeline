...
my $geneBsmlDoc = new BSML::GenePredictionBsml($algorithm, $sourcename);
...
=item my $genePredBsml = new BSML::GenePredictionBsml($algorithm,$sourcename);
...
sub addSequence {
    ...
    if (! defined $self->{seq_files}->{$fastaFile} ) {
        ...
        while(my $line = <$infh>) {
            next if( $line =~ /^\s+$/ );
            if($line =~ /^>((\S+).*)/) {
                if( defined( $ident ) && defined( $seq ) ) {
                    $seq =~ s/\s+//g;
                    $self->{'seqs'}->{$ident}->{'length'} = length( $seq );
                    undef $seq;
                }
                $ident = $2;
...
sub _writeSeqsToDoc {
    ...
    foreach my $seqId (keys %{$self->{'seqs'}}) {
        unless ( exists($self->{'seqs'}->{$seqId}->{'elem'}) ) {
            my $length = $self->{'seqs'}->{$seqId}->{'length'};
            $self->{'seqs'}->{$seqId}->{'elem'} = 
                $doc->createAndAddSequence($seqId, undef, $length, 'dna', 'assembly');
