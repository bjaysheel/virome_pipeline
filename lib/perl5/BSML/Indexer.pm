...
This document refers to $Revision: 615 $ of TIGR::Yank::Indexer.pm.
...
sub index_accessions {
    ...
    if(-e "$filepath.gz"){
	open(DATA, "<:gzip", "$filepath.gz")  or $logger->logdie("Could not open data file $filepath.gz for reading: $!");
    }
    else{
	open (DATA, "<", $filepath)
	    or $logger->logdie("Could not open data file '$filepath' for reading: $!");
    }
    while (my $segment = <DATA>) {
        my @lines = split('\n', $segment);
        my $header = $lines[0];
