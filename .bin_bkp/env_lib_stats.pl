...
if ($options{library} <= 0){
  $getlib = $dbh0->prepare(qq{SELECT name, prefix, server, id
			      FROM library
			      WHERE deleted=0 and server='$options{server}'
			      ORDER BY id});
} else {
  $getlib = $dbh0->prepare(qq{SELECT name, prefix, server, id
			      FROM library
			      WHERE deleted=0 and id=$options{library}
			      ORDER BY id});
}
...
foreach my $lib (@$libraryRSLT) {
  ...
  my $sequence = $dbh->prepare(qq{SELECT distinct query_name, sequenceId
				 FROM blastp
				 WHERE database_name='METAGENOMES'
				   and deleted=0
  ...
  my $sequenceRSLT = $sequence->fetchall_arrayref({});
    ...
    foreach my $seq (@$sequenceRSLT){
        ...
        foreach my $seqId (keys %$seq_lib_struct) {
            ...
            #link = natural=>sum_eval^|^anthropegenic=>sum_eval
            $link =~ s/\^\|\^$//;
        	...
        	print IDOUT "<root>\n";
                ...
                foreach my $key (keys %$sub_cat){
                    ...
                    foreach my $val (keys %$hash_val){
                        $sum_weight += $category->{$feature}->{$key}->{$val}->{'weight'};
                    }
                    ...
                    foreach my $val (keys %$hash_val){
                        ...
                        $tag =~ s/ $//;
                        $tag =~ s/_$//;
                        $key_pairs .= "\t\t<$tag NAME=\"$val\" LABEL=\"$val\"". 
			    " VALUE=\"".  ceil(($category->{$feature}->{$key}->{$val}->{'weight'}/$sum_weight)*100). "\"" .
                            #" IDLIST=\"" . $category->{$feature}->{$key}->{$val}->{'id'}. "\"" .
                            " TAG=\"TAG_" . $tagCount. "\"".
			    " IDFNAME=\"".$fprefix."_IDDOC_".$libId.".xml\"" . 
                        ...
                        print IDOUT "<TAG_".$tagCount." IDLIST=\"$category->{$feature}->{$key}->{$val}->{'id'}\"/>\n";
                        $tagCount++;
                    }
                    $id_list =~ s/,$//;
                    push @{$print_hash{$key}}, {'key' => $key, 'weight' => $sum_weight, 'sub-cat'=>$key_pairs, 'idList'=>$id_list, 'libName'=>$libName, 'tagCount'=>$tagCount};
                ...
                foreach my $k (%print_hash){
                    ...
                    foreach my $g (@$f){
                        ...
                        $g->{key} =~ s/ $//;
                        $g->{key} =~ s/_$//;
                        print OUT "<$cname NAME=\"$g->{key}\" LABEL=\"$g->{key}\"".
			    " VALUE=\"".  ceil(($g->{'weight'}/$total_weight)*100). "\"".
                            #" IDLIST=\"$g->{idList}\"";
                            " TAG=\"TAG_".$g->{tagCount}."\"".
			    " IDFNAME=\"".$fprefix."_IDDOC_".$libId.".xml\"";
                        ...
                        #print id tag and value in seperate file. no nesting required.
                        print IDOUT "<TAG_".$g->{'tagCount'}." IDLIST=\"$g->{'idList'}\"/>\n";
                    }
                }
                print OUT "</root>";
                print IDOUT "</root>";
...
sub bestEvalue{
    ...
    foreach my $field ('lib_prefix','genesis','sphere','ecosystem','extreme','physio_chem_mods','lib_type'){
        my $blast = $dbh->prepare(qq{SELECT m.$field as field, m.lib_prefix, min(b.e_value) as eval, m.lib_shortname
                                   FROM blastp b INNER JOIN mgol_library m on LEFT(b.hit_name,3)=m.lib_prefix
                                   WHERE b.deleted=0
                                    and m.deleted=0
                                    and b.database_name='METAGENOMES'
                                    and b.query_name = '$query_name'
        ...
        foreach my $line (@$blastRSLT){
            ...
            if ($field eq 'lib_prefix'){
                ...
                push @{$env_feature{ $query_name }}, { 'id' => $sequenceId,
                                                      'type' => 'LIBRARY',
                ...
                push @{$env_feature{ $query_name }}, { 'id' => $sequenceId,
                                                      ...
                                                      'from' => $line->{lib_prefix},
                                                      'libName' => $line->{lib_shortname},
                                                      'cat' => uc (!length($line->{field})) ? "Not Extreme" : $line->{field},
                                                      'eval' => $line->{eval} };
