...
#--------------------------------------------------------------------------------------
# program name:   proktigr2chadoudmnt.pl
# authors:        Jay Sundaram and Sam Angiuoli
# date:
# date modified:  2003/10/24 - -r overrides the $ENV{OUTPUT_DIR}
#
# Purpose:        To migrate the prokaryotic organisms from legacy prok database
#                 schema (NON TIGR datasource) into the chado unified data model (whereby, prok data is 
...
proktigr2chadoudmnt.pl - Migrates non-TIGR legacy prok data into Chado schema

=head1 SYNOPSIS

USAGE:  proktigr2chadoudmnt.pl -U username -P password -D source_database -t target_database [-a asmbl_id_list|ALL|-F asmbl_file] [-l log4perl] [-d debug_level] [-h] [-m] [-r outdir]
...
=item B<--log4perl,-l>
    ...
    Optional - Coati::Logger log4perl log file.  Default is /tmp/ntproktigr2chado.pl.log

=item B<--debug_level,-d>

    Optional - Coati::Logger log4perl logging/debugging level.  Default is 0
...
=item B<--man,-m>
    ...
    Displays pod2usage man pages for this script

=item B<--outdir,-r>

    Output directory for the tab delimited .out files
...
#
$log4perl = "/tmp/ntproktigr2chado.pl.log" if (!defined($log4perl));
...
my $database_prefix = "TIGR_ntprok";
...
sub migrate_subfeature_data{
    ...
    $asmbl_id_list_ref   = $parameter_hash->{'asmbl_id_list_ref'}   if (exists $parameter_hash->{'asmbl_id_list_ref'});
    ...
    #----------------------------------
    my $tigr_orf_data = $t2creader->nt_subfeature_data($asmbl_id_list_ref);
    ...
    #
    my ($transcript_cvterm_id, $part_of_cvterm_id, $gene_cvterm_id, $assembly_cvterm_id, $produced_by_cvterm_id, $gene_product_name_cvterm_id);
    my ($tigr_role_cvterm_id, $protein_cvterm_id, $cds_cvterm_id, $exon_cvterm_id);
...
sub migrate_assembly_data{
   ...
   for (my $i=0;$i<$tigr_assembly->{'count'};$i++){
	...
	$t2cwriter->store_assembly_data(
					...
					gb_acc                  => undef, #"", # gb_acc
					chromosome              => undef, #"", # chromosome
					is_public               => undef, #"", # is_public
					ed_date                 => $tigr_assembly->{$i}->{'ed_date'}, 
					name                    => $tigr_organism->{'name'},
					database                => $t2creader->{'_db'},                  # contains name of database eg. 'gbs'
					org_type_is_euk         => 0,                                    # tells BulkSybaseChadoPrismDB whether migrating euk or not
					assembly_cvterm_id      => $assembly_cvterm_id,
	...
	my $assembly_ctr = $tigr_assembly->{'count'} + 1;
	&show_progress("Assembly #$assembly_ctr",$counter,++$row_count,$bars,$total_rows);
    }
    print ("\n");
...
sub print_usage {
    ...
    print STDERR "  -a|--asmbl_id_list          = Optional - comma-separated list of assembly identifiers or ALL to specify all assembly identifiers\n";
    print STDERR "  -F|--asmbl_file             = Optional - file containing newline separated list of assembly identifiers\n";
    print STDERR "  -l|--log4perl               = Optional - Coati::Logger log4perl log file\n";
