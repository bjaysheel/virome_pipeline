...
USAGE:  createSelectRecordCountsSQL.pl [--analysis_id] [--algorithm] --database --database_type [-d debug_level] [--feature_id] [--for_workflow] [-h] [--is_obsolete] [--logfile] [-m] [--organism_id] [--outdir] --password --server --username
...
if (!defined($logfile)){
    $logfile = '/tmp/createSelectRecordCountsSQL.pl.log';
...
if ($criticalParamCtr == 0 ){
    $logger->warn("Neither --analysis_id, --algorithm, --feature_id, --is_obsolete, or --organism_id were defined");
}
...
my $notdir = $outdir . '/not';
my $withdir = $outdir . '/with';
my $alldir = $outdir . '/all';

&createDirectory($notdir);
&createDirectory($withdir);
&createDirectory($alldir);
...
if (defined($analysis_id)){
    ...
    $prism->createSelectStmtsForAnalysisId($analysis_id, $withdir, $for_workflow);
    
    $prism->createSelectStmtsNotForAnalysisId($analysis_id, $notdir, $for_workflow);
}
    ...
    $prism->createSelectStmtsForAlgorithm($algorithm, $withdir, $for_workflow);
    
    $prism->createSelectStmtsNotForAlgorithm($algorithm, $notdir, $for_workflow);
}
    ...
    $prism->createSelectStmtsForFeatureId($feature_id, $withdir, $for_workflow);
    
    $prism->createSelectStmtsNotForFeatureId($feature_id, $notdir, $for_workflow);
}
    ...
    $prism->createSelectStmtsForFeatureIsObsolete( $withdir, $for_workflow);
    
    $prism->createSelectStmtsNotForFeatureIsObsolete( $notdir, $for_workflow);
}
    ...
    $prism->createSelectStmtsForOrganismId($organism_id, $withdir, $for_workflow);
    
    $prism->createSelectStmtsNotForOrganismId($organism_id, $notdir, $for_workflow);
}

$prism->createSelectStmtsForAllTables($alldir, $for_workflow);
...
sub printUsage {
    ...
    "  --logfile        = Optional - log4perl log file (default is /tmp/createSelectRecordCountsSQL.pl.log)\n".
