...
sub recordFindings {
    ...
    $report .= "Tally:1\tTally-description:BSML element counts were\n";
	...
	$report .= "$profileHeadings->{$counterType} $count\n";
    }
	...
	if (exists $masterProfileLookup->{$file}->{$lookupType}){
	    ...
	    my $subReport = &report($profileLookupHeadings->{$lookupType}, $masterProfileLookup->{$file}->{$lookupType}, $tallyCount);
	    $report .= $subReport;
	}
	else {
	    $report .= "One of the files did not contain elements necessary to compute counts for this type '$lookupType'\n";
	}
    }
...
##----------------------------------------------------------------------
sub report {
    my ($msg, $lookup, $tallyCount) = @_;
    
    my $reportString = "\nTally:$tallyCount\tTally-description:$msg\n";

    foreach my $key ( sort keys %{$lookup}){
	$reportString .= "$key\t\t$lookup->{$key}\n";
    }
