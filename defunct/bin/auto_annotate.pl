...
my $length_cutoff = 70;                     #Default cutoff for ber hit (length of hit)
...
foreach my $input(@inputFiles) {
    ...
    foreach my $polypeptide ( keys %polypeptideIds ) { 
        #next unless($polypeptide eq 'mmycoides.polypeptide.665408.1');
...
sub getBerHeader {
    ...
    #If we have the wrong number of columns, that's okay.
    return \@cols;
}
...
sub getHMMEvidence {
    ...
    $logger->logdie("Could not find hmmpfam bsml output file for $polyId") 
        unless($hmmBsmlFile);

    #Now parse the file.
    my $twig = new XML::Twig( TwigHandlers => 
                              { 'Seq-pair-alignment' => sub { 
                                  &handleHMMSpa(@_, \@hmmMatches)} }         
                              ); 
...
sub getBestHMMMatch {
    ...
    foreach my $hmmMatch ( @hmmMatches ) {
        foreach my $domain ( @{$hmmMatch->{'domains'}} ) {
            ...
            if($hmmMatch->{'iso_type'} =~ /equivalog/ && 
               $domain->{'domain_score'} >= $hmmMatch->{'trusted_cutoff'}) {
            ...
            #Okay, to determine what the rank is, check autoAnnotate.data.  
            #It holds a fun data structure.
            if(!$winner->{'rank'} || 
...
sub annotation2bsml {
    ...
    foreach my $polyid (keys %{$finalAnnotes}) {
        my $feat = $doc->createAndAddFeature($featTable, $ber_map->{$polyid}, 
                                             $ber_map->{$polyid}, 'transcript');
        ...
        foreach my $term (keys %{$finalAnnotes->{$polyid}}) {
            ...
            if($term =~ /go_id/) {
                ...
                $doc->createAndAddBsmlAttribute( $feat, 'gene_symbol', 
                                                 $finalAnnotes->{$polyid}->{$term} );
            } elsif( $term eq 'com_name' ) {
                $doc->createAndAddBsmlAttribute( $feat, 'gene_product_name', 
                                                 $finalAnnotes->{$polyid}->{$term} );
            } elsif( $term eq 'annotation_from' ) {
                $doc->createAndAddBsmlAttribute( $feat, 'gene_product_name_source', 
                                                 $finalAnnotes->{$polyid}->{$term} );
            } else { 
