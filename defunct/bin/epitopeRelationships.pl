...
epitopeRelationships.pl - Parse BSML document and produce a tab-delimited feature_relationship.out BCP file for insertion into chado database

=head1 SYNOPSIS

USAGE:  epitopeRelationships.pl -D database --database_type -P password -U username --server -b bsmldoc [-d debug_level] [-h] [--id_repository] [-l logfile] [-m] [-o outdir] [--rel_type_id]
...
eptiopeRelationships.pl - Parse BSML document and produce a tab-delimited feature_relationship.out BCP file for insertion into chado database
...
use Coati::Logger;
use Chado::FeatureRelationship::BCP::Util;

use constant DEFAULT_REL_TYPE_ID => 17;
...
my $util = new Chado::FeatureRelationship::BCP::Util(prism=>$prism,
						     type_rel=>$relTypeId);
if (!defined($util)){
    $logger->logdie("Could not instantiate Chado::".
		    "FeatureRelationship::BCP::Util");
}
...
if (!defined($parser)){
    $logger->logdie("Could not instantiate BSML".
		    "::BsmlParserSerialSearch");
}

print "Parsing all Sequence Feature-group sections ".
"in BSML file '$bsmldoc'\n";

$parser->parse($bsmldoc);


$util->processFeatureGroups(lookup=>$featureGroupRef);
...
sub checkCommandLineArguments {
    ...
    if (!defined($relTypeId)){
	$relTypeId = DEFAULT_REL_TYPE_ID;
	print STDERR "--rel_type_id was not specified and therefore was set to '$relTypeId'\n";
    }
