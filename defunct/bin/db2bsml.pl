...
db2bsml.pl - Retrieve data from legacy annotation database and write content to BSML file
...
my ($username, $password, $outfile, $database, $debug_level, $help,
    ...
    $idGenType, $asmbl_id, $outIdMapFile, $inIdMapDir, $inIdMapFile,
    $vendor);
...
my $converter = $factory->create(type               => $database,
...
sub checkCommandLineArguments {
    ...
    if (!defined($asmbl_id)){
	print STDERR "--asmbl_id was not specified\n";
	$fatalCtr++;
    }
    ...
    if (!defined($outfile)){
	$outfile = File::Basename::basename($0) . '_' . $database . '_' . $asmbl_id . '_assembly.bsml';
	print STDERR "--outfile was not specified and ".
	"therefore was set to '$outfile'\n";
    }

    if (!defined($logfile)){
	$logfile = File::Basename::basename($0) . '_' . $database . '_' . $asmbl_id . '_assembly.bsml.log';
	...
	$outIdMapFile = File::Basename::basename($0) . '_' . $database . '_' . $asmbl_id . '_assembly.out.idmap';
	print STDERR "--out-id-map-file was not specified and ".
	"therefore was set to '$outIdMapFile'\n";
    }

    if (!defined($inIdMapFile)){
	$inIdMapFile = File::Basename::basename($0) . '_' . $database . '_' . $asmbl_id . '_assembly.out.idmap';
