...
mmi_run_pipeline.pl - A script allowing VIROME-Ergatis pipeline instantiation
...
USAGE: virome_454_fasta_run_pipeline.pl 
            ...
            --username=ewommack
            --database=diag1
            --mgol=/path/to/mgol.btab
            --uniref=/path/to/uniref.list
...
B<--mgol,-m>
    Path to MGOL btab results

B<--uniref,-n>
    Path to UniRef btab list
...
my $mgol = $options{'mgol'};
my $uniref = $options{'uniref'};
...
## Archiver
my $archiver_config = new Ergatis::ConfigFile(
...
$archiver_config->setval('parameter', '$;LOCATION$;', $database );
$archiver_config->setval( 'parameter', '$;USER_NAME$;', $username);
$archiver_config->setval( 'parameter', '$;ID$;', $library_id);
$archiver_config->RewriteConfig();

## Clean Expand Btab UniRef
my $ce_uniref_config = new Ergatis::ConfigFile(
    -file => "$options{repository_root}/workflow/runtime/clean_expand_btab/" . $pipeline->id . "_uniref/clean_expand_btab.uniref.user.config");
$ce_uniref_config->setval('input', '$;INPUT_FILE_LIST$;', $uniref );
$ce_uniref_config->RewriteConfig();

## Clean Expand Btab MGOL
my $ce_mgol_config = new Ergatis::ConfigFile(
    -file => "$options{repository_root}/workflow/runtime/clean_expand_btab/" . $pipeline->id . "_mgol/clean_expand_btab.mgol.user.config");
$ce_mgol_config->setval('input', '$;INPUT_FILE$;', $mgol );
$ce_mgol_config->RewriteConfig();
