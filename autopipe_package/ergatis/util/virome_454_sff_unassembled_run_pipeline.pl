...
## sffinfo
my $sffinfo_config = new Ergatis::ConfigFile(
    -file => "$options{repository_root}/workflow/runtime/sffinfo/" . $pipeline->id . "_default/sffinfo.default.user.config");
$sffinfo_config->setval('input', '$;INPUT_FILE$;', $fasta );
...
my @LOCATION = qw( clean_expand_btab.mgol clean_expand_btab.rna clean_expand_btab.uniref db-load-library.default db-load-nohit.default db-to-lookup.seq-lookup db-upload.mgol-blast db-upload.orfs db-upload.orfs_nuc db-upload.rna-blast db-upload.rna-clean db-upload.rna db-upload.sequence_relationship-prep db-upload.trna db-upload.uniref env_lib_stats.default fxnal_count_chart_creator_all.default fxnal_count_chart_creator.default gen_lib_stats.default libraryHistogram.default sequence_relationship-prep.default tie_seq_id.default viromeClassification.default viromeTaxonomyXML.default );
